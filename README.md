...
- **Pi extensions** — small extensions for the [Pi coding agent](https://pi.dev): [model prompt](https://github.com/lukemelnik/pi-model-prompt) for per-model prompt addenda, [session recap](https://github.com/lukemelnik/pi-session-recap) for a configurable one-line session memory widget, and [pi-monitor](https://github.com/lukemelnik/pi-monitor) for tracking live Pi agents and jumping to their tmux panes. Install with `pi install npm:@lukemelnik/pi-model-prompt`, `pi install npm:@lukemelnik/pi-session-recap`, and `pi install npm:@lukemelnik/pi-monitor`.
